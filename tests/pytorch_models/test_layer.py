...
from renda.pytorch_models.layer import DecoderLayer, EncoderLayer, Layer
...
def test_seeding_with_default_init(layer_class, bias):
...
def test_seeding_with_torch_nn_init(layer_class, bias):
    seed = random.randint(min_seed(), max_seed())
...
@pytest.mark.parametrize(
    "bias,tied_weights,delta_seed",
    itertools.product([False, True], [False, True], [0, 1]),
)
def test_tied_and_independent_weights(bias, tied_weights, delta_seed):
    seed = random.randint(min_seed(), max_seed())
    ...
    model = torch.nn.Sequential(
        OrderedDict(
            [
                ...
                ("decoder", DecoderLayer(50, 10, bias=bias, seed=seed + delta_seed)),
            ]
        )
    )
    ...
    else:  # Independent weights
        if delta_seed == 0:  # Same seed
            assert (model.decoder.weight == model.encoder.weight).all()
        else:  # Different seed
...
def test_activation_args(layer_class):
    # Str
    layer_class(
        in_features=10,
        out_features=50,
