...
from renda.utils.random import (
    fix_seed,
    get_max_seed,
    get_min_seed,
    is_seed,
...
def test_get_min_seed():
    assert get_min_seed() == np.iinfo(np.uint32).min


def test_get_max_seed():
    assert get_max_seed() == np.iinfo(np.uint32).max


def test_is_seed():
    assert is_seed(get_min_seed())
    assert is_seed(get_max_seed())

    # Too small or too large?
    assert not is_seed(get_min_seed() - 1)
    assert not is_seed(get_max_seed() + 1)
...
def test_process_seed():
    assert process_seed(get_min_seed()) == get_min_seed()
    assert process_seed(get_max_seed()) == get_max_seed()

    seed = [0, 1, 2]

    assert process_seed(seed, allow_sequence=True) == seed
    assert process_seed(seed, allow_sequence=True, length=len(seed)) == seed
    ...
    with pytest.raises(ValueError):
        # Incorrect length
        process_seed(seed, allow_sequence=True, length=len(seed) - 1)
    with pytest.raises(ValueError):
        # Sequence with invalid seed (None)
        process_seed([0, None, 2], allow_sequence=True)


def test_fix_seed():
    seeds = [get_min_seed(), get_max_seed()]
    for seed in seeds:
        assert is_seed(seed)
        seed = fix_seed(seed)
        assert is_seed(seed)

    seeds = [get_min_seed() - 1, get_max_seed() + 1]
    for seed in seeds:
